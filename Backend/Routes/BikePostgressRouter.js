...
router.post('/vehicle/BikeData', (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        ...
        res.json({ message: "User Bike Data posted to PostgreSQL database" });
      }
    }
  );
}
...
//   return hash;
// }
router.get('/vehicle/getBikedata', (req, res) => {
    console.log('Hit GET /vehicle/getBikedata'); // This was incorrect before - said "getcardata"
    
    const { registration_number } = req.query;
    console.log('Query params received:', req.query); // Add this for debugging
    
    let fetch_query = 'SELECT * FROM "bikedata"';
    const values = [];
